...
This is the open-source self-hosted edition of Vasty Shop. The original codebase
...
## What you need to run Vasty Shop
...
STORAGE_BUCKET_DEFAULT=vasty-shop-uploads
...
SMTP_FROM=Vasty Shop <noreply@your-domain.com>
```
...
The vasty-shop AI services already have an `aiProvider` getter alias that
...
- `db.createConnectAccount / getConnectOnboardingLink / getConnectAccountStatus / getConnectDashboardLink`

These are for paying out to **vendors** (shop owners on the marketplace),
NOT platform billing. To wire up: install `stripe`, get your Stripe secret
key, and replace the stub method bodies in `database.service.ts` with
`stripe.accounts.create({type: 'express', ...})` etc. Set:
```
STRIPE_SECRET_KEY=sk_live_...
STRIPE_CONNECT_REFRESH_URL=https://your-app.com/vendor/onboarding/refresh
STRIPE_CONNECT_RETURN_URL=https://your-app.com/vendor/onboarding/complete
```
